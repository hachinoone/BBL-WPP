...
class mutiDimensionSVR():
    ...
    def fit(self,x,y):
        ...
        for i in range(steps):
            # print(i)
            self.model.append(SVR(C=self.C,epsilon=self.e,gamma=self.gamma).fit(x, y[:,i]))
        return self
    def predict(self,x):
...
def tuneHyperParameter(wfNum,wtNum,wl=60,mask='LASSO',deviceNum=None, trX=None, trY=None, vlX=None,vlY=None, dl=None):

    def objective(trial):

        C = trial.suggest_loguniform('C', 0.1, 2)
        e= trial.suggest_uniform('e', 0.01, 0.1)
        gamma = trial.suggest_categorical('gamma', ['scale', 'auto'])

        regressor = mutiDimensionSVR(C=C,epsilon=e,gamma=gamma).fit(trX, trY)
        vY = dl.renorm(vlY)
        predY = dl.renorm(regressor.predict(vlX))
        score = rmseloss(predY, vY)
        return score
...
def main(feat_name, exp_path):
    for wfid in [1]:
        ...
        for wt in range(wtnumber):
            ...
            f = open(saving_path2 + '/wtemb_wt_train' + str(wt), 'rb')
            trX_p = pickle.load(f)
            ...
            f = open(saving_path2 + '/wtemb_wt_vali' + str(wt), 'rb')
            vX_p = pickle.load(f)
            ...
            f = open(saving_path2 + '/wtemb_wt_test' + str(wt), 'rb')
            teX_p = pickle.load(f)
            teX = np.concatenate((teX, teX_p), axis=1)
            f.close()
            HP=tuneHyperParameter(wfid, wt, trX=trX, trY=trY, vlX=vX, vlY=vY, dl=dl)
            clf = mutiDimensionSVR(C=HP['C'], epsilon=HP['e'], gamma=HP['gamma']).fit(tX, tY)
            pred_teY=clf.predict(teX)        
            out_renorm = dl.renorm(pred_teY)
            true_renorm = dl.renorm(teY)
            pr.append(out_renorm)
            tr.append(true_renorm)
            ...
            data_save(RMSE_test_mat, saving_path + '/RMSE_test_mat_' + feat_name + '_svr_comb.csv')
            data_save(MAPE_test_mat, saving_path + '/MAPE_test_mat_' + feat_name + '_svr_comb.csv')
        print(a, b)
        pr=np.concatenate(pr, axis=0)
        f = open(saving_path + '/' + feat_name + '_svr_comb_pred.txt', 'wb')
        pickle.dump(pr, f)
