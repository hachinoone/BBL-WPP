...
def result_test(wt, batch_size, md, dl, saving_path, target_use, RMSE_mat, MAPE_mat):
    ...
    for p in range(batch_number):
        ...
        if (p < batch_number - 1):
            ...
            x_train = torch.tensor(train[idx.reshape(-1)], dtype = torch.float, device=device).view(batch_size, time_len, scale, n_wt, n_feat)
            ...
            x_train = torch.tensor(train[idx_l.reshape(-1)], dtype = torch.float, device=device).view(left, time_len, scale, n_wt, n_feat)
        ...
        if p == 0:
            out = md(wt, x_train, x_train2, device).cpu().detach().data.numpy()
            emb = md.emb1(wt, x_train, x_train2, device).cpu().detach().data.numpy()
            true = y_train
        else:
            out = np.concatenate([out, md(wt, x_train, x_train2, device).cpu().detach().data.numpy()], axis=0)
            emb = np.concatenate([emb, md.emb1(wt, x_train, x_train2, device).cpu().detach().data.numpy()], axis=0)
    ...
    pickle.dump(emb, open(saving_path + '/emb_wt_' + target_use + str(wt), 'wb'))
    print("RMSE is " + str(RMSE))
    print("MAPE is " + str(MAPE))
    RMSE_mat[wt, :] = RMSE
    MAPE_mat[wt, :] = MAPE

    data_save(RMSE_mat, saving_path + '/RMSE_'+ target_use +'_mat.csv')
    data_save(MAPE_mat, saving_path + '/MAPE_'+ target_use +'_mat.csv')
    return RMSE_mat, MAPE_mat

def result_val(wt, batch_size, md, dl, target_use):
    train, train_10, batch_number = dl.dnn_data_generator(tgwtid=wt, batch_size=batch_size, target_use=target_use)
...
def model_train(wt, batch_size, model, dl, learn_rate, lr_decay, print_step, saving_path, epochs):
    ...
    for epoch in range(epochs):
        ...
        for p in range(batch_number):
            if (index[p] < batch_number - 1):
                idx = add + batch_size * index[p] * scale
            ...
            out = model(wt, x_train, x_train2, device)
            model.zero_grad()
        ...
        train_error[epoch] = avg_loss
        model.eval()
        score = result_val(wt, batch_size, model, dl, 'vali')
        ...
        if (score < best_score):
            ...
            if ~torch.isnan(loss):
                torch.save(best_model.state_dict(), saving_path + "/dnn_wt" + str(wt) + ".pkl")
            else:
                print('error!!!!')
                sys.exit(0)

        score2 = result_val(wt, batch_size, model, dl, 'test')
        test_error[epoch] = score2
        #print(avg_loss)
        data_save(train_error, saving_path+'/loss_record_wt'+str(wt)+'.csv')
        data_save(val_error, saving_path+'/val_record_wt'+str(wt)+'.csv')
        data_save(test_error, saving_path+'/test_record_wt'+str(wt)+'.csv')
...
def main():
    for wfid in [1]:
        ...
        for wt in range(wtnumber):
            ...
            model = LSTMAttn0(time_len, scale, n_wt, n_feat, hidden_size, pred_len).to(device)
            #print('wt', wt)
            model.train()
            model = model.to(device)
            ...
            model = model_train(wt, batch_size, model, dl, learn_rate, lr_decay, print_step, saving_path, epochs)
            end_time = time.time()
            train_time[wt] = end_time - start_time
            data_save(train_time, saving_path + '/training_time.csv')
            print("Computing validation error")
            model.eval()
            RMSE_train_mat, MAPE_train_mat = result_test(wt, batch_size, model, dl, saving_path,
                                                         target_use='train', RMSE_mat=RMSE_train_mat,
                                                         MAPE_mat=MAPE_train_mat)
            RMSE_vali_mat, MAPE_vali_mat = result_test(wt, batch_size, model, dl, saving_path,
                                                       target_use='vali', RMSE_mat=RMSE_vali_mat,
                                                       MAPE_mat=MAPE_vali_mat)

            RMSE_test_mat, MAPE_test_mat = result_test(wt, batch_size, model, dl, saving_path,
