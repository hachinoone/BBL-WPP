...
def result_test(trX, trY, batch_size, md, saving_path, target_use, RMSE_mat, MAPE_mat, dl, wt):
    ...
    data_save(RMSE_mat, saving_path + '/RMSE_' + target_use + '_mat_' + feat_name + '_dnn_single.csv')
    data_save(MAPE_mat, saving_path + '/MAPE_' + target_use + '_mat_' + feat_name + '_dnn_single.csv')
...
def main(feat_name, exp_path):
    for wfid in [1]:
        ...
        f = open(saving_path + '/' + feat_name + '_dnn_single_pred.txt', 'wb')
        pickle.dump(pr, f)
