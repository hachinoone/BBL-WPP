...
def main(feat_name, exp_path):
    for wfid in [1]:
        ...
        for wt in range(wtnumber):
            ...
            data_save(RMSE_test_mat, saving_path + '/RMSE_test_mat_' + feat_name + '_svr_single.csv')
            data_save(MAPE_test_mat, saving_path + '/MAPE_test_mat_' + feat_name + '_svr_single.csv')
    
        pr=np.concatenate(pr, axis=0)
        f = open(saving_path + '/' + feat_name + '_svr_single_pred.txt', 'wb')
        pickle.dump(pr, f)
