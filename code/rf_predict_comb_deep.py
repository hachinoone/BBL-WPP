...
def main(feat, exp_path):
    for wfid in [1]:
        ...
        for wt in range(wtnumber):
            ...
            data_save(RMSE_test_mat, saving_path + '/RMSE_test_mat_' + feat_name + '_rf_comb.csv')
           data_save(MAPE_test_mat, saving_path + '/MAPE_test_mat_' + feat_name + '_rf_comb.csv')
            ipt = clf.feature_importances_
            feat[0] = feat[0] + np.sum(ipt[:len1])
            feat[1] = feat[1] + np.sum(ipt[len1:])
        print(feat)
        np.savetxt(saving_path + '/' + feat_name + '_rf_comb_feat.txt', feat)
        pr=np.concatenate(pr, axis=0)
        f = open(saving_path + '/' + feat_name + '_rf_comb_pred.txt', 'wb')
        pickle.dump(pr, f)
