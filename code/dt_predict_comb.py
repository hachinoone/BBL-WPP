...
epochs = 100
#feat_name = 'lstm'
max_depth_set = [5, 8, 15, 30, 50]
min_samples_leaf_set = [50, 100, 200, 400, 500]
...
def main(feat_name, exp_path):
    for wfid in [1]:
        ...
        for wt in range(wtnumber):
            ...
            for max_depth in max_depth_set:
                for min_samples_leaf in min_samples_leaf_set:
                    ...
                    if (cost < best_cost):
                        best_cost = cost
                        best_params = params
            clf = DecisionTreeRegressor(max_depth=best_params['max_depth'], min_samples_leaf=best_params['min_samples_leaf']).fit(tX, tY)
            ...
            data_save(RMSE_test_mat, saving_path + '/RMSE_test_mat_' + feat_name + '_dt_comb.csv')
            data_save(MAPE_test_mat, saving_path + '/MAPE_test_mat_' + feat_name + '_dt_comb.csv')
            ipt = clf.feature_importances_
            feat[0] = feat[0] + np.sum(ipt[:len1])
            feat[1] = feat[1] + np.sum(ipt[len1:])
        print(feat)
        np.savetxt(saving_path + '/' + feat_name + '_dt_comb_feat.txt', feat)   
        pr=np.concatenate(pr, axis=0)
        f = open(saving_path + '/' + feat_name + '_dt_comb_pred.txt', 'wb')
        pickle.dump(pr, f)
